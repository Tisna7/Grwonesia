  ...
  private function request(array $payload, ?string $system = null, string $kind = 'text'): ?array
  {
    ...
    foreach ($modelsToTry as $currentModel) {
      ...
      try {
        ...
        Log::warning("GeminiClient: request model {$currentModel} gagal", [
          'status' => $response->status(),
          'body' => mb_substr($response->body(), 0, 300),
        ]);
      } catch (Throwable $e) {
        Log::warning("GeminiClient: exception model {$currentModel}", ['message' => $e->getMessage()]);
      }
    }

    $this->logRequest($kind, false, $startedAt, null, 'All models failed or rate-limited');
