...
@section('content')
  <div class="grid gap-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
      ...
      <x-stat-card label="Omzet Bulan Ini" :value="rupiah($stats['omzet_month'])" :delta="$stats['omzet_month_delta']"
        icon="trending-up" />
      <x-stat-card label="Pesanan Bulan Ini" :value="number_format($stats['orders_month'], 0, ',', '.')"
        icon="file-text" />
    ...
    @if ($govPrograms->isNotEmpty())
      <div class="glass-card p-6 border-indigo-500/25">
        <div class="flex items-center gap-2">
          <div class="icon-tile !w-9 !h-9"
            style="background: linear-gradient(145deg, rgba(79,70,229,0.28), rgba(79,70,229,0.06)); border-color: rgba(129,140,248,0.3); color: #A5B4FC;">
            ...
            <div class="glass-card !bg-primary-950/30 p-4 flex flex-col">
              ...
              <p class="mt-1 text-[11px] text-slate-400">
                ...
                @if ($program->starts_at)
                  <span
                    class="block mt-0.5">{{ $program->starts_at->format('d M Y') }}{{ $program->ends_at ? ' – ' . $program->ends_at->format('d M Y') : '' }}</span>
                @endif
              </p>
        ...
        <div class="flex items-center justify-between">
          ...
          @if ($stats['pending_orders'] > 0)
            <a href="{{ route('business.orders.index') }}"
              class="badge-pill bg-amber-500/15 text-amber-400 border-amber-500/40">
