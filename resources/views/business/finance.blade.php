  ...
  <div class="grid gap-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="glass-card p-6 lg:col-span-2">
        ...
        <p class="text-[11px] text-slate-500">Berdasarkan rata-rata omzet & biaya 30 hari terakhir, dikurangi biaya tetap
          harian.</p>
      ...
      <div class="glass-card p-6">
        ...
        <form method="POST" action="{{ route('business.finance.fixed-cost') }}" class="mt-4 space-y-3">
          @csrf
          <input type="number" name="monthly_fixed_cost" value="{{ (int) $business->monthly_fixed_cost }}" min="0"
            step="50000" class="form-input">
        ...
        <div class="mt-5 pt-4 border-t border-primary-400/10 space-y-2 text-xs text-slate-400">
          <div class="flex justify-between"><span>Rata-rata omzet/hari</span><span
              class="font-mono text-slate-200">{{ rupiah($analysis['avg_daily_revenue']) }}</span></div>
          <div class="flex justify-between"><span>Rata-rata HPP/hari</span><span
              class="font-mono text-slate-200">{{ rupiah($analysis['avg_daily_cost']) }}</span></div>
          <div class="flex justify-between"><span>Biaya tetap/hari</span><span
              class="font-mono text-slate-200">{{ rupiah($analysis['monthly_fixed_cost'] / 30) }}</span></div>
        </div>
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-primary-400/5">
            @forelse ($analysis['product_margins'] as $row)
              <tr class="hover:bg-primary-600/5">
                ...
                <td class="px-5 py-3 text-right">
                  <span
                    class="badge-pill {{ $row['margin_pct'] >= 40 ? 'bg-emerald-500/15 text-emerald-400 border-emerald-500/40' : ($row['margin_pct'] >= 25 ? 'bg-amber-500/15 text-amber-400 border-amber-500/40' : 'bg-rose-500/15 text-rose-400 border-rose-500/40') }}">
                    {{ $row['margin_pct'] }}%
                  </span>
                </td>
              </tr>
            @empty
              <tr>
                <td colspan="5" class="px-5 py-8 text-center text-xs text-slate-500">Belum ada penjualan dalam 30 hari
                  terakhir.</td>
              </tr>
            @endforelse
          </tbody>
