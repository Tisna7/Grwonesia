  ...
  <div class="grid grid-cols-1 xl:grid-cols-3 gap-6" x-data="policySimulator()" x-init="simulate()">
    ...
    <div class="glass-card p-6">
      <div class="flex items-center gap-2">
        <span
          class="w-8 h-8 rounded-lg ai-gradient flex items-center justify-center text-white text-sm ai-active-pulse">🎛️</span>
        <h3 class="text-sm font-bold ai-gradient-text">Parameter Kebijakan</h3>
      </div>
      <p class="mt-2 text-xs text-slate-400">Geser parameter — proyeksi dihitung real-time dari data omzet riil platform.
      </p>
        ...
        <div>
          <div class="flex justify-between text-xs">
            ...
            <span class="font-mono text-primary-300"
              x-text="params.pelatihan_batch + ' batch (' + (params.pelatihan_batch * 25) + ' UMKM)'"></span>
          </div>
      ...
      <button class="btn-primary w-full mt-6 text-xs" :class="loadingNarrative && 'btn-shimmer'" @click="getNarrative()"
        :disabled="loadingNarrative || !{{ $aiConfigured ? 'true' : 'false' }}">
