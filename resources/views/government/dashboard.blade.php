...
<div class="grid gap-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <x-stat-card label="UMKM Aktif (30 Hari)" :value="number_format($stats['active_businesses'], 0, ',', '.')" icon="zap"/>
        <x-stat-card label="Omzet Regional 30 Hari" :value="rupiah($stats['omzet_30'])" :delta="$stats['omzet_30_delta']" icon="wallet"/>
        <x-stat-card label="Volume Transaksi 30 Hari" :value="number_format($stats['transactions_30'], 0, ',', '.')" icon="file-text"/>
    </div>
                ...
                @forelse ($stats['city_hotspots'] as $spot)
                    <div>
                        <div class="flex justify-between text-xs">
                            ...
                            <span class="font-mono text-slate-400">{{ rupiah($spot['omzet']) }} · {{ $spot['transactions'] }} trx · {{ $spot['businesses'] }} UMKM</span>
                        </div>
                ...
                @forelse ($stats['category_trends'] as $trend)
                    <div class="flex items-center justify-between text-sm">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <span class="badge-pill {{ $trend['change'] >= 0 ? 'bg-emerald-500/15 text-emerald-400 border-emerald-500/40' : 'bg-rose-500/15 text-rose-400 border-rose-500/40' }}">
