...
<body class="min-h-screen antialiased text-slate-100 flex overflow-x-hidden"
      x-data="grownesiaUserDashboard()"
      x-init="initDashboard()">
    <div class="ambient-glow"></div>

    <!-- LEFT SIDEBAR NAVIGATION (LOGGED-IN USER LAYOUT) -->
    <aside class="w-64 glass-card !rounded-none border-r border-primary-400/15 min-h-screen flex flex-col justify-between shrink-0 sticky top-0 h-screen z-40 hidden md:flex">
        
        <!-- Sidebar Brand & User Profile Card -->
        <div class="p-5 border-b border-primary-400/10 space-y-4">
            <div class="flex items-center justify-between">
                <a href="{{ route('landing') }}" class="flex items-center gap-3 group">
                    <span class="w-9 h-9 rounded-xl ai-gradient flex items-center justify-center text-white font-extrabold text-lg shadow-lg shadow-purple-600/40" style="font-family: 'Space Grotesk'">G</span>
                    <div>
                        <span class="block text-lg font-bold tracking-tight text-white leading-none font-heading">Grownesia</span>
                        <span class="block mt-1 text-[10px] font-bold uppercase tracking-[0.16em] text-purple-300/80">Portal Pembeli</span>
                    </div>
                </a>
            </div>
            ...
            <button @click="activeTab = 'profile'" 
                ...
                <div class="flex items-center gap-2.5 min-w-0">
                    <div class="w-7 h-7 rounded-xl bg-gradient-to-tr from-purple-600 to-pink-600 flex items-center justify-center text-white font-extrabold text-xs shrink-0 shadow">
        ...
        <div class="flex-1 py-5 px-3.5 space-y-1 overflow-y-auto text-xs font-semibold">
            ...
            <div class="text-[10px] uppercase font-extrabold tracking-wider text-purple-400/70 px-3 pt-1 pb-2">Navigasi Utama</div>
            ...
            <button @click="activeTab = 'ai-gift'" 
                    ...
                    class="w-full px-3.5 py-2.5 rounded-xl transition flex items-center gap-3">
                <svg class="w-4 h-4 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5a2 2 0 10-2 2h2z"/></svg>
            ...
            <div class="text-[10px] uppercase font-extrabold tracking-wider text-purple-400/70 px-3 pt-4 pb-2">Belanja & Transaksi</div>
            ...
            <button @click="activeTab = 'cart'" 
                    ...
                    class="w-full px-3.5 py-2.5 rounded-xl transition flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <svg class="w-4 h-4 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 0a2 2 0 100 4 2 2 0 000-4z"/></svg>
                    <span>Keranjang Belanja</span>
                </div>
                <span x-show="cartTotalCount > 0" x-text="cartTotalCount" class="bg-fuchsia-500 text-white text-[10px] font-extrabold px-2 py-0.5 rounded-full shadow"></span>
            </button>
            ...
            <button @click="activeTab = 'favorites'" 
                ...
                <span x-show="favorites.length > 0" x-text="favorites.length" class="bg-red-500/20 text-red-300 text-[10px] font-extrabold px-2 py-0.5 rounded-full border border-red-500/30"></span>
            </button>
            ...
            <button @click="activeTab = 'orders'" 
                ...
                <span x-text="ordersHistory.length" class="bg-emerald-500/20 text-emerald-300 text-[10px] font-extrabold px-2 py-0.5 rounded-full border border-emerald-500/30"></span>
            </button>
            ...
            <div class="text-[10px] uppercase font-extrabold tracking-wider text-purple-400/70 px-3 pt-4 pb-2">Pengaturan Akun</div>
        ...
        <div class="p-4 border-t border-purple-500/15 bg-purple-950/80">
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit" class="w-full py-2.5 px-3 rounded-xl bg-purple-900/40 hover:bg-red-950/70 border border-purple-500/20 hover:border-red-500/40 text-purple-300 hover:text-red-200 text-xs font-bold transition flex items-center justify-center gap-2 group">
                    <svg class="w-4 h-4 text-purple-400 group-hover:text-red-300 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/></svg>
    ...
    <div class="flex-1 min-w-0 flex flex-col min-h-screen">
        ...
        <header class="sticky top-0 z-30 glass-card !rounded-none border-b border-primary-400/10 px-6 py-4 flex items-center justify-between backdrop-blur-xl">
            <div class="flex items-center gap-4">
                <h2 class="text-lg font-bold text-white font-heading">
                    Selamat datang, <span x-text="userProfile.name"></span>!
                </h2>
                <span class="hidden lg:inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold bg-purple-500/20 text-purple-300 border border-purple-400/30">
            ...
            <div class="flex items-center gap-3 relative">
                ...
                <a href="{{ route('business.dashboard') }}" class="px-3 py-2 rounded-xl bg-purple-900/60 hover:bg-purple-800 border border-primary-400/30 text-purple-200 text-xs font-semibold flex items-center gap-1.5 transition shadow-md">
                    <svg class="w-4 h-4 text-fuchsia-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
    ...
    <div class="fixed bottom-6 right-6 z-50">
        ...
        <div x-show="showFloatingAiWidget" x-cloak 
             ...
             class="absolute bottom-16 right-0 w-80 sm:w-96 rounded-3xl bg-slate-950/95 border border-purple-400/40 shadow-2xl backdrop-blur-2xl overflow-hidden flex flex-col justify-between" style="box-shadow: 0 20px 50px rgba(124, 58, 237, 0.5);">
            ...
            <div class="p-3 bg-purple-950/60 border-b border-purple-500/20 space-y-1.5">
                ...
                <div class="flex flex-wrap gap-1.5">
                    <button @click="sendAiQuery('Saya ingin hadiah untuk ibu umur 50 tahun.')" class="px-2.5 py-1 rounded-full bg-purple-900/80 hover:bg-purple-800 border border-purple-400/30 text-purple-200 text-[11px] font-semibold transition">
                        🎁 Hadiah Ibu 50th
                    </button>
                    <button @click="sendAiQuery('Saya punya budget Rp300.000 untuk hampers.')" class="px-2.5 py-1 rounded-full bg-purple-900/80 hover:bg-purple-800 border border-purple-400/30 text-purple-200 text-[11px] font-semibold transition">
                        💰 Budget Rp300.000
                    </button>
                    <button @click="sendAiQuery('Kopi gula aren atau kopi klepon mana yang lebih disukai?')" class="px-2.5 py-1 rounded-full bg-purple-900/80 hover:bg-purple-800 border border-purple-400/30 text-purple-200 text-[11px] font-semibold transition">
                        ⚖️ Komparasi Kopi
                    </button>
                </div>
            </div>
            ...
            <div class="p-4 space-y-3 max-h-72 overflow-y-auto text-xs">
                <template x-for="(msg, idx) in chatMessages" :key="'fmsg-'+idx">
                    <div :class="msg.sender === 'user' ? 'flex justify-end' : 'flex justify-start'">
                        <div :class="msg.sender === 'user' ? 'bg-[#5B21B6] text-white rounded-2xl rounded-tr-none p-3 max-w-[85%]' : 'bg-purple-950/80 border border-purple-400/30 text-purple-100 rounded-2xl rounded-tl-none p-3 max-w-[90%] space-y-2'" style="box-shadow: inset 0 1px 0 rgba(255,255,255,0.05)">
                            ...
                            <template x-if="msg.products && msg.products.length">
                                <div class="pt-2 space-y-2 border-t border-purple-500/20">
                                    <template x-for="p in msg.products" :key="'fp-'+p.id">
                                        <div class="p-2 rounded-xl bg-purple-900/50 border border-purple-500/30 flex items-center justify-between gap-2">
                                            <div class="flex items-center gap-2 min-w-0">
                                                <img :src="p.image" class="w-9 h-9 rounded-lg object-cover border border-purple-400/30">
                                                <div class="min-w-0">
                                            ...
                                            <button @click="addToCart(p)" class="px-2.5 py-1 rounded-lg bg-purple-600 hover:bg-purple-500 text-white font-bold text-[10px] shrink-0">
                                                + Beli
                                            </button>
                                        </div>
                                    </template>
                                    <button @click="addBundleToCart()" class="w-full py-1.5 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 text-white font-bold text-[11px] shadow-md hover:opacity-90 transition">
            ...
            <!-- Input Form -->
            <form @submit.prevent="sendAiQuery(customChatInput)" class="p-3 bg-purple-950/90 border-t border-purple-500/20 flex gap-2">
                <input type="text" x-model="customChatInput" placeholder="Tanyakan rekomendasi AI..." class="flex-1 px-3 py-2 rounded-xl bg-slate-900 border border-purple-400/30 text-xs text-white placeholder-purple-400/60 focus:outline-none focus:border-purple-400">
                <button type="submit" class="px-3.5 py-2 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs shadow-md">Kirim</button>
            </form>
        </div>
    </div>
</body>
