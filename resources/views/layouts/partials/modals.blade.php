...
<div x-show="showAiDrawer"
    ...
    <div @click.away="showAiDrawer = false" class="w-full max-w-md bg-purple-card border-l border-purple-500/30 h-full flex flex-col shadow-2xl">
        <div class="p-5 border-b border-purple-500/20 bg-purple-950/80 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl bg-purple-600/30 border border-purple-400/40 flex items-center justify-center text-purple-300 shadow-md">
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-4 text-xs" id="chatContainer">
            <div class="p-3.5 rounded-2xl bg-purple-900/50 border border-purple-500/30 text-purple-200 space-y-2">
                ...
                <p>Halo {{ Auth::user()->name ?? 'Budi' }}! Saya asisten belanja berbasis AI Anda. Ketik pertanyaan Anda atau pilih opsi cepat:</p>
                <div class="pt-1 flex flex-col gap-1.5">
                    <button @click="sendAiQuery('Saya ingin hadiah untuk ibu umur 50 tahun.')" class="text-left p-2 rounded-xl bg-purple-950/60 hover:bg-purple-800/60 border border-purple-400/30 text-purple-200 text-xs transition flex items-center justify-between">
            ...
            <template x-for="(msg, index) in chatMessages" :key="index">
                <div :class="msg.sender === 'user' ? 'flex justify-end' : 'flex justify-start'">
                    <div :class="msg.sender === 'user' ? 'bg-purple-600 text-white rounded-2xl rounded-tr-none p-3.5 max-w-[85%]' : 'bg-purple-950/80 border border-purple-500/30 text-purple-100 rounded-2xl rounded-tl-none p-3.5 max-w-[90%] space-y-2'">
                        <p x-text="msg.text"></p>
        ...
        <div class="p-4 border-t border-purple-500/20 bg-purple-950/80">
            <form @submit.prevent="sendAiQuery(customChatInput)" class="flex gap-2">
                <input type="text" x-model="customChatInput" placeholder="Tanyakan saran produk atau hadiah..." class="flex-1 px-3.5 py-2.5 rounded-xl bg-purple-900/60 border border-purple-500/30 text-xs text-white placeholder-purple-400/60 focus:outline-none focus:border-purple-400">
                <button type="submit" class="px-4 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-semibold text-xs">Kirim</button>
            </form>
        </div>
    </div>
</div>
...
<div x-show="showCartDrawer"
    ...
    <div @click.away="showCartDrawer = false" class="w-full max-w-md bg-purple-card border-l border-purple-500/30 h-full flex flex-col shadow-2xl">
        <div class="p-5 border-b border-purple-500/20 bg-purple-950/80 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <svg class="w-6 h-6 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/></svg>
        ...
        <div class="flex-1 overflow-y-auto p-5 space-y-4">
            <template x-if="cart.length === 0">
                <div class="text-center py-16 text-purple-300/60 space-y-3">
                    <svg class="w-16 h-16 mx-auto text-purple-500/30" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/></svg>
                    <p>Keranjang Anda masih kosong.</p>
                    <button @click="showCartDrawer = false" class="px-4 py-2 rounded-xl bg-purple-600 text-white text-xs font-semibold">Mulai Belanja</button>
                </div>
            </template>
            ...
            <template x-for="(item, idx) in cart" :key="item.product.id">
                <div class="p-3.5 rounded-xl bg-purple-900/40 border border-purple-500/20 flex gap-3 items-center">
                    <img :src="item.product.image" class="w-14 h-14 rounded-lg object-cover border border-purple-400/30">
                    <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-1.5 bg-purple-950/70 rounded-lg p-1 border border-purple-500/30">
                        <button @click="updateQty(idx, -1)" class="w-5 h-5 rounded text-purple-300 hover:bg-purple-800 text-xs flex items-center justify-center">-</button>
        ...
        <div x-show="cart.length > 0" class="p-5 border-t border-purple-500/20 bg-purple-950/80 space-y-4">
            ...
            <button @click="openCheckoutModal()" class="w-full py-3 rounded-xl bg-gradient-to-r from-purple-600 to-fuchsia-600 hover:from-purple-500 hover:to-fuchsia-500 text-white font-bold text-sm transition shadow-lg shadow-purple-900/50">
...
<div x-show="showCheckoutModal"
     ...
     style="display: none;">
    <div @click.away="showCheckoutModal = false" class="w-full max-w-xl bg-purple-card border border-purple-500/30 rounded-2xl overflow-hidden shadow-2xl space-y-5 p-6">
        ...
        <div class="space-y-4 text-xs">
            <div>
                ...
                <textarea x-model="userProfile.address" class="w-full p-2.5 rounded-xl bg-purple-950/80 border border-purple-500/30 text-purple-100" rows="2"></textarea>
            </div>

            <div>
                <label class="block font-semibold text-purple-200 mb-1">Metode Pembayaran</label>
                <div class="grid grid-cols-3 gap-2">
                    <label class="p-2.5 rounded-xl border border-purple-500/40 bg-purple-900/40 cursor-pointer flex flex-col items-center justify-center gap-1 hover:border-purple-400">
        ...
        <button @click="processPaymentSuccess()" class="w-full py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold text-sm transition shadow-lg flex items-center justify-center gap-2">
...
<div x-show="showSuccessModal"
     ...
     style="display: none;">
    <div class="w-full max-w-lg bg-purple-card border border-purple-400/40 rounded-3xl p-6 text-center space-y-4 shadow-2xl relative overflow-hidden">
        <div class="w-16 h-16 rounded-full bg-emerald-500/20 border-2 border-emerald-400 text-emerald-400 flex items-center justify-center mx-auto shadow-lg shadow-emerald-500/20">
            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>
        </div>

        <h3 class="text-2xl font-extrabold text-white font-heading">Pembayaran Berhasil! 🎉</h3>
        <p class="text-xs text-purple-200/90">Terima kasih atas pesanan Anda. Transaksi Anda membantu menggerakkan roda ekonomi UMKM lokal Indonesia!</p>
...
<div x-show="showProductDetailModal && selectedProductDetail"
     ...
     style="display: none;">
    <div @click.away="showProductDetailModal = false" class="w-full max-w-2xl bg-purple-card border border-purple-500/30 rounded-3xl overflow-hidden shadow-2xl space-y-5 p-6">
        <template x-if="selectedProductDetail">
            <div class="space-y-5">
                <div class="flex items-center justify-between border-b border-purple-500/20 pb-3">
                    <div class="flex items-center gap-2">
                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-purple-800/40 text-purple-300 border border-purple-400/30" x-text="selectedProductDetail.category"></span>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-12 gap-5 items-center">
                    <div class="sm:col-span-5 relative h-56 rounded-2xl overflow-hidden bg-purple-950 border border-purple-400/30">
                        <img :src="selectedProductDetail.image" class="w-full h-full object-cover">
                        <button @click="toggleFavorite(selectedProductDetail)" class="absolute top-3 right-3 p-2 rounded-full bg-purple-950/80 text-white border border-purple-400/30 hover:scale-110 transition">
                            <svg class="w-5 h-5" :class="isFavorite(selectedProductDetail.id) ? 'text-red-500 fill-current' : 'text-purple-300'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="sm:col-span-7 space-y-3">
                        <div class="text-xs font-bold text-purple-400 uppercase tracking-widest" x-text="'Penjual: ' + selectedProductDetail.umkm + ' (' + selectedProductDetail.region + ')'"></div>
                        ...
                        <div class="flex items-baseline gap-2 pt-1">
                            <span class="text-2xl font-extrabold text-white" x-text="formatRupiah(selectedProductDetail.price)"></span>
                            <span class="text-xs text-purple-400/60 line-through" x-text="formatRupiah(selectedProductDetail.original_price)"></span>
                        </div>
                    </div>
                </div>

                <div class="flex items-center gap-3 pt-3 border-t border-purple-500/20">
                    <button @click="addToCart(selectedProductDetail); showProductDetailModal = false" class="flex-1 py-3 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs transition flex items-center justify-center gap-2">
                        + Tambah ke Keranjang
                    </button>
                    <button @click="addToCart(selectedProductDetail); showProductDetailModal = false; openCheckoutModal()" class="flex-1 py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-bold text-xs transition">
...
<div x-show="showProfileModal"
     class="fixed inset-0 z-50 overflow-y-auto bg-purple-950/85 backdrop-blur-md flex items-center justify-center p-4"
     style="display: none;">
    <div @click.away="showProfileModal = false" class="w-full max-w-lg bg-purple-card border border-purple-500/30 rounded-3xl p-6 space-y-4 shadow-2xl">
        <div class="flex items-center justify-between border-b border-purple-500/20 pb-3">
        ...
        <form @submit.prevent="saveProfile()" class="space-y-3 text-xs">
            <div>
                ...
                <input type="text" x-model="userProfile.name" class="w-full p-2.5 rounded-xl bg-purple-950/80 border border-purple-500/30 text-white">
            </div>

            <div>
                <label class="block font-semibold text-purple-200 mb-1">Alamat Email</label>
                <input type="email" x-model="userProfile.email" class="w-full p-2.5 rounded-xl bg-purple-950/80 border border-purple-500/30 text-white">
            </div>

            <div>
                <label class="block font-semibold text-purple-200 mb-1">Nomor Telepon / WhatsApp</label>
                <input type="text" x-model="userProfile.phone" class="w-full p-2.5 rounded-xl bg-purple-950/80 border border-purple-500/30 text-white">
            </div>

            <div>
                <label class="block font-semibold text-purple-200 mb-1">Alamat Pengiriman Utama</label>
                <textarea x-model="userProfile.address" class="w-full p-2.5 rounded-xl bg-purple-950/80 border border-purple-500/30 text-white" rows="2"></textarea>
            </div>

            <button type="submit" class="w-full py-3 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs transition shadow-lg">
...
<div x-show="showRolePreviewModal"
     class="fixed inset-0 z-50 overflow-y-auto bg-purple-950/85 backdrop-blur-md flex items-center justify-center p-4"
     style="display: none;">
    <div @click.away="showRolePreviewModal = false" class="w-full max-w-2xl bg-purple-card border border-purple-400/40 rounded-3xl p-6 space-y-5 shadow-2xl">
        <div class="flex items-center justify-between border-b border-purple-500/20 pb-4">
            <div class="flex items-center gap-3">
                <span class="px-3 py-1 rounded-full text-xs font-bold bg-purple-500/30 text-purple-300 border border-purple-400/40 uppercase" x-text="previewRoleTitle"></span>
