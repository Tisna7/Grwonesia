...
class RoleMiddlewareTest extends TestCase
{
  ...
  {
    $consumer = User::factory()->create(['role' => 'user', 'status' => 'terverifikasi']);

    $this->actingAs($consumer)->get('/business/dashboard')->assertRedirect(route('user.dashboard'));
  }

  public function test_business_user_can_access_dashboard(): void
  {
    $user = User::factory()->create(['role' => 'business', 'status' => 'terverifikasi']);
